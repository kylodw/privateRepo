...
final class DecodeHelper<Transcode> {
  ...
  <Data> LoadPath<Data, ?, Transcode> getLoadPath(Class<Data> dataClass) {
    //其中resourceClass是Object.class，transcodeClass则是Drawable.class
  ...
  List<LoadData<?>> getLoadData() {
    if (!isLoadDataSet) {
      isLoadDataSet = true;
      loadData.clear();
      //通过glideContext.getRegistry()获取到 Registry 对象，在gilde的构造方法中创建的，注册添加了很多解析器
      //getModelLoaders
      ...
      for (int i = 0, size = modelLoaders.size(); i < size; i++) {
        ...
        LoadData<?> current =
            //buildLoadData  其中有一个HttpUriLoader  这里举例  例如HttpUriLoader
