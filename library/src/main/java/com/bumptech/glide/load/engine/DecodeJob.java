...
class DecodeJob<R> implements DataFetcherGenerator.FetcherReadyCallback,
  ...
  private void runWrapped() {
    //在build的时候runReason  被赋值INITIALIZE
    switch (runReason) {
      case INITIALIZE:
  ...
  private void runGenerators() {
    ...
    while (!isCancelled && currentGenerator != null
        //这里执行startNext方法，当前的SourceGenerator对象
  ...
  private void decodeFromRetrievedData() {
    ...
    try {
      //这里解码封装已经完成  LazyBitmapDrawableResource
  ...
  private <Data> Resource<R> decodeFromData(DataFetcher<?> fetcher, Data data,
    ...
    try {
      ...
      //这个fetcher的用意，主要是去关闭输入流和HttpUrlConnection的。
      fetcher.cleanup();
    }
  }
  ...
  private <Data> Resource<R> decodeFromFetcher(Data data, DataSource dataSource)
      throws GlideException {
    //z这个data.class 目前是InputStrem.class   解码过程，获取解码器
  ...
  private <Data, ResourceType> Resource<R> runLoadPath(Data data, DataSource dataSource,
    ...
    //它调用glideContext.getRegistry()获取Registry对象，然后调用Registry的getRewinder获取装置器
    DataRewinder<Data> rewinder = glideContext.getRegistry().getRewinder(data);
    try {
      // ResourceType in DecodeCallback below is required for compilation to work with gradle.
      //传递了一个new DecodeCallback 对象的回调类
      return path.load(
