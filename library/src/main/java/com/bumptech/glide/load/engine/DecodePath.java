...
public class DecodePath<DataType, ResourceType, Transcode> {
  ...
  public Resource<Transcode> decode(DataRewinder<DataType> rewinder, int width, int height,
    ...
    //解码成功返回BitmapResource
    Resource<ResourceType> decoded = decodeResource(rewinder, width, height, options);

    Resource<ResourceType> transformed = callback.onResourceDecoded(decoded);
    //transcoder是BitmapDrawableTranscoder
  ...
  private Resource<ResourceType> decodeResourceWithList(DataRewinder<DataType> rewinder, int width,
    ...
    //这里首先遍历decoders集合，分别的获取到ResourceDecoder解码器，还记得我们的decoders都包含哪些解码器吗？
    // 没错主要包含两种：BitmapDrawable.class和GifDrawable.class
    for (int i = 0, size = decoders.size(); i < size; i++) {
      ResourceDecoder<DataType, ResourceType> decoder = decoders.get(i);
      try {
        DataType data = rewinder.rewindAndGet();
        //通过decoder.handles(data, options)方法来过滤掉不相匹配的解码器
        if (decoder.handles(data, options)) {
          //然后通过rewinder.rewindAndGet()获取我们的InputStream数据流：
          data = rewinder.rewindAndGet();
          //decoder是BitmapDrawableDecoder
