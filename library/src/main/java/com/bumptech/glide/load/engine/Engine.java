...
public class Engine implements EngineJobListener,
  ...
  public synchronized <R> LoadStatus load(
    ...
    //获取key，通过keyFactory工厂来构建一个EngineKey对象，key关联着model，也就是url，
    // 它很根据model，view的宽高等等属性来构建一个EngineKey对象，这个对象可以用来指定缓存地址，
    // 可以用来从缓存中查找资源等。
    EngineKey key = keyFactory.buildKey(model, signature, width, height, transformations,
        resourceClass, transcodeClass, options);
    //查找ActiveResourceCache,先从弱引用中取图片
    EngineResource<?> active = loadFromActiveResources(key, isMemoryCacheable);
    if (active != null) {
      //如果有缓存直接调用onResourceReady 设置图片
    ...
    //创建EngineJob，EngineJob对象负责开启线程去加载资源，并且加载得资源后转换到主线程并进行回调
    ...
    //创建DecodeJob，DecodeJob是真正的执行者，它就是去网络加载资源的地方，
    // EngineJob开启线程，真正执行的是DecodeJob，DecodeJob之后完毕之后叫道EngineJob去分发回调。这就是这两个类的关系。
