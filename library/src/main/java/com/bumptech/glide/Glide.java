...
public class Glide implements ComponentCallbacks2 {
  ...
  /**
   * 获取应用中带注解的GlideModule（annotationGeneratedModule），这里要解释一下GlideModule：用户自定义glide配置模块，用来修改默认的glide配置信息。
   * 如果这个为空或者可配置menifest里面的标志为true，则获取menifest里面配置的GlideModule模块（manifestModules）。
