...
public class RequestBuilder<TranscodeType> extends BaseRequestOptions<RequestBuilder<TranscodeType>>
    ...
    protected RequestBuilder(@NonNull Glide glide, RequestManager requestManager, Class<TranscodeType> transcodeClass, Context context) {
        ...
        //这个根据类型转变，类型（asDrawable对应Drawable）等等
     ...
     * those values that are explicitly set in the given {@link RequestOptions} object. If you need to
     ...
     * if it's set on this {@link RequestBuilder} via {@link RequestOptions#fallback(android.graphics.drawable.Drawable)}
     ...
     * {@link RequestBuilder} at position 0 has the {@link RequestBuilder} at position 1 applied as
     * using its thumbnail method, the {@link RequestBuilder} at position 1 has the {@link
     ...
     * an empty array of {@link RequestBuilder} thumbnails is equivalent to calling {@link
     ...
     * pass {@link Bitmap}s into Glide. If you have a custom way to obtain {@link Bitmap}s that is not
     * supported by Glide by default, consider registering a custom {@link
     ...
     * {@link Bitmap}s into Glide using this method . If you have a custom way to obtain {@link
     * Bitmap}s that is not supported by Glide by default, consider registering a custom {@link
     * com.bumptech.glide.load.model.ModelLoader} or {@link com.bumptech.glide.load.ResourceDecoder}
     * instead of using this method.
     *
     * <p>The {@link DiskCacheStrategy} is set to {@link DiskCacheStrategy#NONE}. Previous calls to
     ...
     *
     * <p> Note - this method caches data using only the given String as the cache key. If the data
     * is a Uri outside of your control, or you otherwise expect the data represented by the given
     * String to change without the String identifier changing, Consider using {@link
     ...
     *
     * <p> Note - this method caches data at Uris using only the Uri itself as the cache key. The
     * data represented by Uris from some content providers may change without the Uri changing, which
    ...
    private <Y extends Target<TranscodeType>> Y into(
        ...
        //request与target相互绑定
        //target在这里一般是imageview  除非你设置tag
        //targetListener 为null
