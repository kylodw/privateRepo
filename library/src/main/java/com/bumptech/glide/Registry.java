...
public class Registry {
  ...
  /**
   * Registers the given {@link Encoder} for the given data class (InputStream, FileDescriptor
   * etc).
   *
   ...
   * {@link Encoder} being used for {@link android.content.res.AssetFileDescriptor.AutoCloseInputStream},
   ...
   * the registered {@link ResourceDecoder} will run first. You can use the {@link
   ...
   *
   * @param buckets The list of bucket identifiers in order from highest priority to least
   *                priority.
   * @see #append(String, Class, Class, ResourceDecoder)
   * @see #prepend(String, Class, Class, ResourceDecoder)
   */
   ...
   * data type.
   *
   * <p>The {@link ResourceEncoder} will be used both for the exact resource class and any
   * subtypes.
   * For example, registering an {@link ResourceEncoder} for {@link android.graphics.drawable.Drawable}
   * (not recommended) will result in the {@link ResourceEncoder} being used for {@link
   * android.graphics.drawable.BitmapDrawable} and {@link com.bumptech.glide.load.resource.gif.GifDrawable}
  ...
  private <Data, TResource, Transcode> List<DecodePath<Data, TResource, Transcode>> getDecodePaths(
    ...
    //registeredResourceClasses集合中分别对应的是Bitmap.class，BitmapDrawable.class和GifDrawable.class的三种Class对象。
    ...
    //这是对registeredResourceClasses集合的再次匹配，我们知道transcodeClasses实际上是Drawable.class，
    // 在registeredResourceClasses集合中只有BitmapDrawable.class和GifDrawable.class是继承Drawable的，
    // 由此我们得出registeredTranscodeClasses包含BitmapDrawable.class和GifDrawable.class两种Class对象。
    ...
    for (Class<TResource> registeredResourceClass : registeredResourceClasses) {
      ...
      for (Class<Transcode> registeredTranscodeClass : registeredTranscodeClasses) {
        ...
        //通过上面获取的registeredResourceClass和registeredTranscodeClass获取到transcoder转化器，
        // 主要是从transcoderRegistry集合中通过get方法获取，而transcoderRegistry又包含哪些转化器呢？
  ...
  public <X> DataRewinder<X> getRewinder(@NonNull X data) {
    //dataRewinderRegistry在Registry构造方法中创建的对象
    //而且在Glide中进行了注册  register();
